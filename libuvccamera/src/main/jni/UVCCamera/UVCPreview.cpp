...
static const char* gDeviceName = nullptr; // Declare a global variable to store the device name
...
//                    c_start = clock();
                    ...
                    cv::putText(cvFrame, text, cv::Point(20, 35), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(255, 128, 128), 2);
//                    cv::putText(cvFrame, text, cv::Point(20, 70), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(255, 128 , 128),1 );
//                    c_end = clock();
...
Java_com_serenegiant_usb_UVCCamera_nativeSendDeviceNameToNative(JNIEnv *env, jobject thiz,
                                                                jstring device_name) {
    if (gDeviceName) {
        env->ReleaseStringUTFChars(device_name, gDeviceName);
        gDeviceName = nullptr;
    }

    gDeviceName = env->GetStringUTFChars(device_name, nullptr);

    // Return the device_name back to native layer
    return device_name;
